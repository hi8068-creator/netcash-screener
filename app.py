...
# 決算短信・財務の確認先(株探: 業績/財務を百万円表示し、決算短信へのリンクもある)
...
def to_display(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "コード" in d.columns:
        code4 = d["コード"].astype(str).str.replace(".T", "", regex=False)
        d["決算短信"] = KABUTAN_BASE + code4
...
st.dataframe(
    ...
    hide_index=True,
    column_config={
        "決算短信": st.column_config.LinkColumn(
            "決算短信/財務(株探)",
            help="株探の財務ページ。業績・財務が百万円表示で、決算短信PDFへのリンクもあります。",
            display_text="開く",
        ),
...
st.caption(
    ...
    "「円」の絶対額に正規化されるため、銘柄間で単位は揃っています。右端リンクから実際の決算短信と照合できます。"
)
