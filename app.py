...
DISPLAY_ORDER = [
    ...
    "市場", "業種根拠", "規模",
    "流動資産(百万円)", "投資有価証券(百万円)", "負債(百万円)", "非支配株主持分(百万円)",
    "売上トレンド",
...
BEGINNER_COLUMNS = [
    "コード", "銘柄名", "注意", "新業種", "ネットキャッシュ比率", "時価総額(億円)", "PER", "自己資本比率(%)",
...
def to_display(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    d = df.copy()
    for col in ["流動資産", "投資有価証券", "負債", "非支配株主持分"]:
        if col in d.columns:
            d[f"{col}(百万円)"] = (
...
with st.sidebar:
    ...
    exclude_warned = True
    if "注意" in st.session_state.df.columns:
        exclude_warned = st.checkbox(
            "⚠注意銘柄を除外", value=Q("noflag", True, bool),
            help="上場廃止が決定/審査中の銘柄(JPX公式)や、暗号資産を大量保有する銘柄を隠します。"
                 "これらは「割安に見えるだけの罠」のことが多いため、既定でON。",
        )
...
if exclude_warned and "注意" in df.columns:
    df = df[df["注意"].fillna("").astype(str).str.strip() == ""]
...
with tab_help:
    ...
    st.markdown(
        ...
        "- **非支配株主持分** … 連結子会社・連結ファンドのうち\"他人の取り分\"。"
        "これを引かないと、ファンド運営会社などが実態より割安に見えてしまうため控除する。\n"
        "- **注意列(⚠)** … 割安に見えるだけの\"罠\"の可能性。"
        "**整理＝上場廃止が決定**、**監理＝上場廃止の審査中/TOB等の確認中**(いずれもJPX公式の一覧)、"
        "**暗号資産保有＝**流動資産の中身がビットコイン等(価格変動と希薄化のリスク)。"
        "既定では非表示にしており、サイドバーの「⚠注意銘柄を除外」をOFFにすると表示される。\n"
